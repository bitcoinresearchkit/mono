...
const OP_RETURN_OUTPUT_DESCRIPTION: &str = "Outputs whose locking script begins with `OP_RETURN`.";
...
fn assert_audited_descriptions() {
    ...
    for suffix in [
        "1h", "24h", "3d", "1w", "8d", "9d", "12d", "13d", "2w", "21d", "26d", "1m", "34d", "50d",
        "55d", "2m", "9w", "12w", "89d", "3m", "14w", "111d", "144d", "6m", "26w", "200d", "9m",
        "350d", "12m", "1y", "14m", "2y", "26m", "3y", "200w", "4y", "5y", "6y", "8y", "9y", "10y",
        "12y", "14y", "26y",
    ] {
    ...
    for (prefix, metric) in [
        ...
        for (name, aggregation) in [
            ...
            assert_eq!(
                info.description.as_deref(),
                Some(format!("{TX_VERSION_COUNTS_DESCRIPTION} {metric} {aggregation}").as_str()),
                "wrong description for {name}"
            );
        }
            ...
            .zip(WINDOW_DESCRIPTIONS)
        {
            for (kind, aggregation) in [
                ...
                assert_eq!(
                    info.description.as_deref(),
                    Some(
                        format!("{TX_VERSION_COUNTS_DESCRIPTION} {metric} {aggregation} {window}")
                            .as_str()
                    ),
                    "wrong description for {name}"
                );
    ...
    assert!(
        ...
        "undocumented mappings series: {undocumented_mappings:#?}"
    );
    ...
    for pool in pools().iter() {
        ...
        for (window, window_description) in windows.clone() {
            ...
            let expected =
                format!("{POOL_BLOCKS_DESCRIPTION} {ROLLING_SUM_DESCRIPTION} {window_description}");
        ...
        for (suffix, unit) in amount_units {
            ...
            let block_expected = format!("{POOL_REWARDS_DESCRIPTION} {BLOCK_DESCRIPTION} {unit}");
