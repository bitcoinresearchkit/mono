...
const GENERATED_OUTPUTS: &[(&str, &str)] = &[
    ...
    (
        "crates/bitview_client/src/lib.rs",
        "crates/bitview_client/src/lib.rs",
    ),
    (
...
pub fn main() -> Result<()> {
    ...
    let check = match args.as_slice() {
        ...
        _ => bail!(
            "usage: cargo run -p bitview_server --example bindgen --features bindgen [-- --check]"
        ),
    };
...
fn output_paths(root: &Path) -> bitview_bindgen::ClientOutputPaths {
    bitview_bindgen::ClientOutputPaths::new()
        .rust(root.join("crates/bitview_client/src/lib.rs"))
