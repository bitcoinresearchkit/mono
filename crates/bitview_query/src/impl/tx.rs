...
impl Query {
    ...
    #[inline]
    fn resolve_tx_index(&self, txid: &Txid) -> brk_error::Result<TxIndex> {
        self.indexer()
            .stores()
    ...
    pub fn txid_by_index(&self, index: TxIndex) -> brk_error::Result<Txid> {
    ...
    pub fn resolve_tx(&self, txid: &Txid) -> brk_error::Result<(TxIndex, Height)> {
    ...
    #[inline]
    fn confirmed_status_height(&self, tx_index: TxIndex) -> brk_error::Result<Height> {
    ...
    #[inline]
    fn confirmed_status_at(&self, height: Height) -> brk_error::Result<TxStatus> {
    ...
    #[inline]
    fn block_hash_and_time(&self, height: Height) -> brk_error::Result<(BlockHash, Timestamp)> {
    ...
    fn lookup_tx<R>(
        ...
        indexed: impl FnOnce(TxIndex) -> brk_error::Result<R>,
    ) -> brk_error::Result<R> {
    ...
    pub fn transaction(&self, txid: &Txid) -> brk_error::Result<Transaction> {
        self.lookup_tx(txid, Transaction::clone, |idx| {
            self.transaction_by_index(idx)
        })
    }

    pub fn transaction_status(&self, txid: &Txid) -> brk_error::Result<TxStatus> {
    ...
    pub fn transaction_raw(&self, txid: &Txid) -> brk_error::Result<Vec<u8>> {
        self.lookup_tx(txid, Transaction::encode_bytes, |idx| {
            self.transaction_raw_by_index(idx)
        })
    }

    pub fn transaction_hex(&self, txid: &Txid) -> brk_error::Result<String> {
        self.lookup_tx(
            txid,
    ...
    pub fn outspend(&self, txid: &Txid, vout: Vout) -> brk_error::Result<TxOutspend> {
        if self.mempool().is_some_and(|m| m.contains_txid(txid)) {
    ...
    pub fn outspends(&self, txid: &Txid) -> brk_error::Result<Vec<TxOutspend>> {
        if let Some(mempool) = self.mempool()
    ...
    fn build_outspend(&self, txin_index: TxInIndex) -> brk_error::Result<TxOutspend> {
        let indexer = self.indexer();
    ...
    fn resolve_tx_outputs(&self, txid: &Txid) -> brk_error::Result<(TxIndex, TxOutIndex, usize)> {
    ...
    fn transaction_by_index(&self, tx_index: TxIndex) -> brk_error::Result<Transaction> {
        Ok(self
    ...
    fn transaction_raw_by_index(&self, tx_index: TxIndex) -> brk_error::Result<Vec<u8>> {
        let indexer = self.indexer();
    ...
    fn transaction_hex_by_index(&self, tx_index: TxIndex) -> brk_error::Result<String> {
        Ok(self
            .transaction_raw_by_index(tx_index)?
            .to_lower_hex_string())
    }

    pub fn broadcast_transaction(&self, hex: &str) -> brk_error::Result<Txid> {
        self.client().send_raw_transaction(hex)
    }

    pub fn merkleblock_proof(&self, txid: &Txid) -> brk_error::Result<String> {
        let (_, height) = self.resolve_tx(txid)?;
    ...
    pub fn merkle_proof(&self, txid: &Txid) -> brk_error::Result<MerkleProof> {
...
#[inline]
pub fn resolve_tx_index(query: &Query, txid: &Txid) -> brk_error::Result<TxIndex> {
    query.resolve_tx_index(txid)
}

#[inline]
pub fn resolve_tx_index_bounded(query: &Query, txid: &Txid) -> brk_error::Result<TxIndex> {
    query.resolve_tx_index_bounded(txid)
}

#[inline]
pub fn confirmed_status_height(query: &Query, tx_index: TxIndex) -> brk_error::Result<Height> {
    query.confirmed_status_height(tx_index)
}

#[inline]
pub fn confirmed_status_at(query: &Query, height: Height) -> brk_error::Result<TxStatus> {
