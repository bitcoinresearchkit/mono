    ...
    ($mode:ty) => {
        impl AddrsVecs<$mode> {
            pub fn addr_readers(&self) -> AddrReaders {
                AddrReaders {
                    p2pk65: self.p2pk65.bytes.reader().into(),
                    p2pk33: self.p2pk33.bytes.reader().into(),
                    p2pkh: self.p2pkh.bytes.reader().into(),
                    p2sh: self.p2sh.bytes.reader().into(),
                    p2wpkh: self.p2wpkh.bytes.reader().into(),
                    p2wsh: self.p2wsh.bytes.reader().into(),
                    p2tr: self.p2tr.bytes.reader().into(),
                    p2a: self.p2a.bytes.reader().into(),
                }
            }
        }
    };
