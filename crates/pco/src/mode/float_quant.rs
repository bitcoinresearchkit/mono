...
pub(crate) fn join_latents<F: Float>(
  ...
  for ((&y, &m), dst) in primary.iter().zip(secondary.iter()).zip(dst.iter_mut()) {
    ...
    dst.write(F::from_latent_ordered(
      (y << k).wrapping_add(lowest_k_bits),
    ));
  }
...
mod test {
  ...
  fn test_join_split_round_trip() {
    ...
    let k: Bitlen = 5;
    let SplitLatents { primary, secondary } = split_latents(&nums, k);
    let primary = primary.downcast::<u64>().unwrap();
    let secondary = secondary.unwrap().downcast::<u64>().unwrap();
    let mut dst = vec![MaybeUninit::<f64>::uninit(); nums.len()];
    join_latents::<f64>(
      k,
      DynLatentSlice::U64(&primary),
      Some(DynLatentSlice::U64(&secondary)),
      &mut dst,
    )
    .unwrap();
    let dst = dst
      .into_iter()
      // SAFETY: join_latents initialized the complete destination.
