...
pub(crate) fn join_latents<T: Number>(
  ...
  for ((&mult, &adj), dst) in primary.iter().zip(secondary.iter()).zip(dst.iter_mut()) {
    ...
    dst.write(T::from_latent_ordered(
      mult.wrapping_mul(base).wrapping_add(adj),
    ));
  }
...
mod tests {
  ...
  fn test_split_join_latents() {
    ...
    let primary = latents.primary.downcast::<u32>().unwrap();
    let secondary = latents.secondary.unwrap().downcast::<u32>().unwrap();
    let mut dst = vec![MaybeUninit::<u32>::uninit(); nums.len()];
