...
use super::{
    Calibration, DayResult, DayUrpds, ModeId, ModeResult, ModeVecs, ModeWeights, Vecs,
...
use crate::{
    ...
    frameworks::{
        self,
        coinflow::{AgeBand, HORIZON_COUNT, HorizonId},
    },
    indexes,
...
impl ModeVecs {
    ...
    fn push(&mut self, result: &ModeResult) {
        self.loss_threshold
            .push(super::LossPercentileId::from_fn(|id| {
                *id.select(&result.loss_threshold)
            }));
        self.prices
            .push(super::PriceBandId::from_fn(|id| *id.select(&result.prices)));
    }
}
