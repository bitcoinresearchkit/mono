...
defines their roles, current technical boundaries, target experience, and the
criteria used to decide whether a capability is ready for AI use.

## Identity

- **Bitcoin Research Kit (BRK)** is the reusable Rust foundation for reading
  Bitcoin Core data, RPC, mempool tracking, storage, domain types, and on-chain
  price inference.
- **Bitview** is the open-source, self-hostable data platform built on BRK. Its
  official public instance is [bitview.space](https://bitview.space).
- **Bitview MCP** is the public MCP endpoint for Bitview at
  [mcp.bitview.space](https://mcp.bitview.space).
- Public material should describe Bitview as **powered by Bitcoin Research
  Kit**.

Requirements for reusable Bitcoin primitives belong to BRK. Requirements for
the plugin platform, public interfaces, clients, and hosted service belong to
Bitview.
...
- Bitview does not hold private keys, sign transactions, control wallets, or
  trade.
...
Bitview is open-source Bitcoin data infrastructure for humans, applications,
and AI, powered by BRK. Its official public instance provides unlimited and
unauthenticated access without requiring an application backend.
...
The core data contract must not depend on a specific AI vendor or model.
Bitview must expose it through standard REST and OpenAPI interfaces and
Streamable HTTP MCP. Maintained JavaScript, Python, and Rust clients should
derive from the same contract.
...
that page and its linked Bitview and BRK resources, a supported coding AI
should be able to produce a working, deployable, single-file Bitcoin
application in under ten minutes, without manual API debugging.
