...
## [v0.12.2](https://github.com/bitcoinresearchkit/brk/releases/tag/v0.12.2) - 2026-09-01

### Performance Improvements

#### `brk_lsm_tree`

- Reworked LSM run normalization to assign every table to its destination before merging each sorted batch once. This preserves newest-overlapping-run ordering while avoiding repeated insertion and sorting for every table during version updates and compaction; generated-layout tests compare the optimized implementation against the reference algorithm ([source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.2/crates/lsm-tree/src/version/optimize.rs))

### Internal Changes

#### `scripts`

- Added interactive npm one-time-password prompting when a terminal is available and `NPM_CONFIG_OTP` is unset. The publisher passes a supplied OTP to npm while preserving unattended publication with bypass-2FA tokens ([source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.2/scripts/js-publish.sh))

#### `benches` and `bitview_client`

- Committed the v0.12.1 M4/TB4 SSD indexer benchmark capture, including progress, timing, memory, disk, and I/O samples, as a reproducible comparison baseline for storage work ([benchmark](https://github.com/bitcoinresearchkit/brk/tree/v0.12.2/benches/bitviewd/run-1788185759671))
- Repointed the generated JavaScript API reference's source locations to the final v0.12.2 source snapshot, keeping every “Defined in” link attached to the code that produced the release ([reference](https://github.com/bitcoinresearchkit/brk/tree/v0.12.2/modules/bitview-client/docs))

[View changes](https://github.com/bitcoinresearchkit/brk/compare/v0.12.1...v0.12.2)

## [v0.12.1](https://github.com/bitcoinresearchkit/brk/releases/tag/v0.12.1) - 2026-08-31

### Breaking Changes

#### `brk_store`, `brk_fjall`, and `brk_lsm_tree`

- Advanced `brk_store`'s Fjall schema marker for the revised LSM table format. A Bitview node upgrading from v0.12.0 treats the old indexer stores as incompatible, removes that plugin's invalid data, and rebuilds the index before publishing it again ([store](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/brk_store/src/lib.rs), [indexer](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_plugin_indexer/src/lib.rs))
- Removed per-data-block checksums and the storage-device sync barriers previously issued for immutable table files and their directories. The embedded LSM contract now requires consistency after an orderly Bitview shutdown but does not promise recovery after a process crash, kernel failure, or power loss ([table writer](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/lsm-tree/src/table/writer/mod.rs), [storage contract](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/docs/storage-performance-backlog.md))
- Removed the public `brk_lsm_tree::Checksum` type, `Tree::approximate_len`, `brk_fjall::Keyspace::approximate_len`, `brk_store::Store::approximate_len`, and `brk_store::Kind::{is_recent,is_random,is_vec}`. `DescriptorTable::{access_for_table,insert_for_table}` was replaced by the single race-safe `access_or_open` entry point ([LSM API](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/lsm-tree/src/lib.rs), [descriptor table](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/lsm-tree/src/descriptor_table.rs), [store kind](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/brk_store/src/kind.rs))

#### `brk_exit`, `vecdb`, and `brk_types`

- Moved the public `Exit` and `ExitGuard` shutdown types out of `vecdb` into the focused `brk_exit` crate and removed their `vecdb` and `brk_types` re-exports. Direct users must depend on and import `brk_exit`; the signal, persistence-guard, and cleanup-callback behavior is unchanged ([source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/brk_exit/src/lib.rs), [guard](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/brk_exit/src/guard.rs))

#### `bitview_client`

- Changed generated Rust series-tree child fields from eagerly constructed node values to public `LazyNode<T>` wrappers. Normal field access continues to work through deref coercion, but code that names the exact field type or constructs generated nodes directly must account for the wrapper ([source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_bindgen/src/generators/rust/tree.rs), [test](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_client/tests/lazy_series_tree.rs))

#### `website` and `website_next`

- Removed the legacy ChatGPT plugin manifests from both web applications as MCP Registry publication became the supported tool-discovery path ([former website manifest](https://github.com/bitcoinresearchkit/brk/blob/v0.12.0/website/.well-known/ai-plugin.json), [former next-site manifest](https://github.com/bitcoinresearchkit/brk/blob/v0.12.0/website_next/.well-known/ai-plugin.json))

### New Features

#### `bitview_plugin_bedrock`, `bitview_compute`, and `brk_types`

- Added daily P05 through P95 Bedrock cost-basis percentile series for both Cointime and Coinflow models, with separate per-coin (BTC-weighted) and per-dollar (acquisition-value-weighted) distributions. The percentile pass reads persisted URPD entries directly without rebuilding their map and independently backfills the new series from validated model state ([source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_plugin_bedrock/src/cost_basis_vecs.rs), [source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/brk_types/src/urpd_raw.rs))
- Exposed the four new percentile families under `bedrock.cost_basis.per_coin` and `bedrock.cost_basis.per_dollar` in generated clients and added matching Per Coin and Per Dollar chart groups to the website. The public catalog grew from 61,764 to 61,992 series ([client](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_client/src/generated.rs), [website](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/website/scripts/options/models/bedrock.js), [catalog](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/website/llms.txt))

#### `bitview_mcp` and `scripts`

- Added the official `io.github.bitcoinresearchkit/bitview` MCP Registry manifest and integrated validation, idempotent lookup, authenticated publication, and post-publish verification into the release pipeline ([manifest](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_mcp/server.json), [source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/scripts/mcp-publish.sh))

### Reliability and Performance

#### `bitview_plugin_indexer`, `brk_store`, `brk_lsm_tree`, and `brk_fjall`

- Strengthened the indexer's shared-store commit boundary: the completion marker is invalidated before mutation, remains absent if an ingest is dropped or fails, and is published only after every related store commit has completed. Missing or malformed checkpoints therefore force safe recovery instead of allowing a partially committed height to resume ([source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_plugin_indexer/src/stores/checkpoint/stores_checkpoint.rs), [tests](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_plugin_indexer/src/stores/checkpoint/tests.rs))
- Added a checksum and strict length validation to the atomically replaced current-version manifest, bounded manifest table counts, validated block handles and compression lengths before allocation, durably created Fjall's initial format marker, and consolidated concurrent descriptor-cache misses onto one shared file descriptor ([recovery](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/lsm-tree/src/version/recovery/mod.rs), [block validation](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/lsm-tree/src/table/block/mod.rs), [descriptor cache](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/lsm-tree/src/descriptor_table.rs))
- Applied LZ4 to default Fjall data blocks from level one onward while leaving level zero uncompressed, and reduced version-update work by sharing untouched levels instead of cloning every run ([Fjall options](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/fjall/src/keyspace/options.rs), [LSM version](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/lsm-tree/src/version/mod.rs))

#### `brk_byteview`

- Kept short `ByteView::fused` results inline and initialized heap allocation headers as complete values, avoiding an allocation for small fused keys and removing partial initialization of the reference-count header ([source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/byteview/src/byteview.rs))

#### `bitview_client`, `bitview_bindgen`, `website`, and `website_next`

- Made generated Rust series-tree nodes and the website's large cohort and chart subtrees lazy, and moved chart-title matching to a compact generated index loaded inside a Web Worker. Explorer lookups no longer wait for chart search, direct explorer links select their option immediately, and external search results open in a separate tab ([Rust generator](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_bindgen/src/generators/rust/tree.rs), [website lazy groups](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/website/scripts/options/lazy.js), [search worker](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/website/scripts/panes/search-worker.js))

### Bug Fixes

#### `brk_lsm_tree`

- Fixed run normalization so every older table stays behind each newer overlapping run, including transitive overlaps. This prevents point lookups from returning a stale value after particular flush and compaction layouts ([source and tests](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/lsm-tree/src/version/optimize.rs))

#### `bitview_compute` and `bitview_client`

- Fixed USD rolling-average series identifiers so the dollar-valued leaf keeps the unsuffixed base name while only the cent-valued leaf receives `_cents`; this prevents generated discovery paths from referring to an unintended `_usd` series name ([source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitview_compute/src/per_block/fiat/lazy_rolling_avg.rs))
- Corrected JavaScript and Python package metadata that still reported v0.11.2 after the v0.12.0 workspace release, then regenerated the clients with the Bedrock paths and lazy Rust tree ([JavaScript](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/modules/bitview-client/package.json), [Python](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/packages/bitview_client/pyproject.toml))

### Internal Changes

#### Workspace, `bitview_bindgen`, and `scripts`

- Changed workspace repository metadata from the former `bitcoinresearchkit/brk` location to the canonical `bitcoinresearchkit/mono` repository, converted binding generation into the `bitview-bindgen` binary, and made the release hook regenerate and verify committed bindings before publication ([workspace](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/Cargo.toml), [generator](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/crates/bitviewd/src/bin/bindgen.rs), [release script](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/scripts/rust-publish.sh))
- Made Rust publication resumable when a selected crate is already present, published `importmap` before its dependent website crate, and parsed crates.io rate-limit retry times with both macOS and GNU date tooling before retrying ([source](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/scripts/rust-publish.sh))

#### `benches` and `docs`

- Split the M4/TB4 SSD Bitview daemon benchmark records into headful and headless runs, added v0.12.0 and v0.12-next headless captures, and documented the benchmark promotion gate, storage durability boundary, rejected tuning attempts, and remaining performance candidates ([benchmarks](https://github.com/bitcoinresearchkit/brk/tree/v0.12.1/benches/bitviewd), [backlog](https://github.com/bitcoinresearchkit/brk/blob/v0.12.1/docs/storage-performance-backlog.md))

[View changes](https://github.com/bitcoinresearchkit/brk/compare/v0.12.0...v0.12.1)
